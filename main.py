...
async def close(ctx):
    if ctx.channel.type != discord.ChannelType.private:
        ...
        elif ctx.author.id in 949325556657311775 and 'ticket' in ctx.channel.name:
            await ctx.channel.delete()
        else:
...
async def bronze(ctx):
 if ctx.channel.type != discord.ChannelType.private:
    ...
    embed=discord.Embed(title="Free Bronze",color=10181046, description=f"**set .gg/PhNPHrJWZ2 in status, you will automatically get a rank Bronze**")
    await ctx.send(embed=embed)
...
async def on_member_update(before, after):
    role_id = 966178143205466152
    role = get(before.guild.roles, id=role_id)
    if '.gg/PhNPHrJWZ2' in str(before.activities):
      if '.gg/PhNPHrJWZ2' in str(after.activities):
        pass
      else:
...
async def help(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
            ...
            embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/919555915831726112/938833095698829423/7283_twitch.png')
            ...
            embed.add_field(name='Twitch Report', value=f'`{prefix}treport (nick)`', inline=True)
            embed.add_field(name='Twitch Troll', value=f'`{prefix}ttroll (nick)`', inline=True)
            embed.add_field(name='Twitch Raid', value=f'`{prefix}traid (nick)`', inline=True)
            embed.add_field(name='Stock', value=f'`{prefix}stock`', inline=True)
            embed.add_field(name='Free Bronze', value=f'{prefix}bronze', inline=True)
